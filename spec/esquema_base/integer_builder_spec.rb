...
RSpec.describe EsquemaBase::Builders::IntegerBuilder do
  ...
  context 'with invalid options' do
    it 'raises an error' do
      expect do
        described_class.new('name', invalid: 'option').build_property
  ...
  context 'with invalid option values' do
    it 'raises an error' do
      expect do
        described_class.new('name', minimum: '1').build_property
      end.to raise_error(TypeError)
    end

    it 'raises an error' do
      expect do
        described_class.new('name', maximum: '10').build_property
      end.to raise_error(TypeError)
    end

    context 'with nil value' do
      it 'raises an error with nil value' do
        prop = described_class.new('name', minimum: nil).build_property
        expect(prop).to eq({ type: 'integer' })
      end
    end
  end
