...
RSpec.describe EasyTalk::Builders::StringBuilder do
  ...
  context 'with invalid keys' do
    it 'raises an error' do
      ...
      expect do
        described_class.new(:name, nil, invalid: 'invalid').build
  ...
  context 'with invalid values' do
    it 'raises an error' do
      expect do
        described_class.new(:name, nil, min_length: 'invalid').build
  ...
  context 'with empty string value' do
    it 'raises a type error' do
      expect do
        described_class.new(:name, nil, min_length: '').build
      end.to raise_error(TypeError)
    end
  end
  ...
  context 'with nil value' do
    it 'does not include the key' do
      prop = described_class.new(:name, nil, min_length: nil).build
      expect(prop).to eq({ type: 'string' })
    end
  end
end
