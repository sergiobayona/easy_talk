...
RSpec.describe EasyTalk::Builders::IntegerBuilder do
  ...
  context 'with invalid option values' do
    it 'raises an error when the type is wrong' do
      ...
      end.to raise_error(EasyTalk::ConstraintError, "Error in property 'name': Constraint 'minimum' expects Integer, but received \"1\" (String).")
    end
    ...
    it "raises an error when the value isn't a number" do
      ...
      end.to raise_error(EasyTalk::ConstraintError, "Error in property 'name': Constraint 'minimum' expects Integer, but received \"a\" (String).")
    end

    it 'raises an error when the value is an empty string' do
      expect do
        described_class.new(:name, minimum: '').build
      end.to raise_error(EasyTalk::ConstraintError, "Error in property 'name': Constraint 'minimum' expects Integer, but received \"\" (String).")
    end
  end
end
