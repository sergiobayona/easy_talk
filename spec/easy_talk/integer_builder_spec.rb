...
RSpec.describe EasyTalk::Builders::IntegerBuilder do
  ...
  context 'with invalid options' do
    it 'raises an error' do
      expect do
        described_class.new(:name, nil, invalid: 'option').build
  ...
  context 'with invalid option values' do
    it 'raises an error' do
      expect do
        described_class.new(:name, nil, minimum: '1').build
      end.to raise_error(TypeError)
    end

    it 'raises an error' do
      expect do
        described_class.new(:name, nil, maximum: '10').build
      end.to raise_error(TypeError)
    end

    context 'with nil value' do
      it 'raises an error with nil value' do
        prop = described_class.new(:name, nil, minimum: nil).build
        expect(prop).to eq({ type: 'integer' })
      end
    end
  end
